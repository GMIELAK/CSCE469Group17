...
apiRouter.get('/', (req, res) => {
    res.send('Password Manager Backend API');
});
...
// Get credentials
apiRouter.get('/credentials', authenticateToken, (req, res) => {
    res.json(credentials[req.user.id] || []);
});

// Add credential
apiRouter.post('/credentials', authenticateToken, (req, res) => {
...
// Update credential
apiRouter.put('/credentials/:id', authenticateToken, (req, res) => {
    const { id } = req.params;
    const { site, username, password, notes } = req.body;
...
// Delete credential
apiRouter.delete('/credentials/:id', authenticateToken, (req, res) => {
    const { id } = req.params;
    const userCredentials = credentials[req.user.id];
    if (!userCredentials) {
        return res.status(404).send('Credentials not found');
